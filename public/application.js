...
/***/ (function(module, exports, __webpack_require__) {
	...
	var surveyEvents = __webpack_require__(13);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onCreateSurvey = function onCreateSurvey(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.createSurvey(data).then(ui.createSurveySuccess).catch(ui.createSurveyFailure);
	};

	var onCreateQuestion = function onCreateQuestion(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.createQuestion(data).then(ui.createQuestionSuccess).catch(ui.createQuestionFailure);
	};

	var onIndexOfSurveys = function onIndexOfSurveys(event) {
	  event.preventDefault();
	  api.indexOfSurveys().then(ui.indexOfSurveysSuccess).catch(ui.indexOfSurveysFailure);
	};

	var onShowAuthUserSurveys = function onShowAuthUserSurveys(event) {
	  event.preventDefault();
	  var userId = $(this).attr('userId');
	  api.showAuthUserSurveys(userId).then(ui.showAuthUserSurveysSuccess).catch(ui.showAuthUserSurveysFailure);
	};

	var onDestroy = function onDestroy(event) {
	  event.preventDefault();
	  var surveyId = $(this).attr('surveyId');
	  api.destroy(surveyId).then(ui.destroySuccess).catch(ui.destroyFailure);
	};
	...
	var onUpdate = function onUpdate(event) {
	  ...
	  var surveyId = $(this).attr('surveyId');
	  var data = getFormFields(event.target);
	  api.update(surveyId, data).then(ui.updateSuccess).catch(ui.updateFailure);
	};

	var onRevealAddQuestion = function onRevealAddQuestion(event) {
	  $('form#create-survey').show();
	  $('#handlebar-target').text('');
	};

	var takeSurvey = function takeSurvey(event) {
	  event.preventDefault();
	  var surveyId = $(this).attr('surveyId');
	  api.surveyQuestions(surveyId).then(ui.takeSurveySuccess).catch(ui.takeSurveyFailure);
	};

	var onSurveyQuestions = function onSurveyQuestions(event) {
	  event.preventDefault();
	  var surveyId = $(this).attr('surveyId');
	  api.surveyQuestions(surveyId).then(ui.surveyQuestionsSuccess).catch(ui.surveyQuestionsFailure);
	};
	...
	var onAnswerQuestion = function onAnswerQuestion(event) {
	  ...
	  data = data === '1';
	  api.answerQuestion(data, id).then(ui.answerSuccess).catch(ui.answerFailure);
	};

	var onGetQuestionData = function onGetQuestionData(event) {
	  var id = $(this).attr('questionId');
	  api.getQuestionData(id).then(ui.getQuestionDataSuccess).catch(ui.getQuestionDataFailure);
	};

	var onDeleteQuestion = function onDeleteQuestion(event) {
	  event.preventDefault();
	  var id = $(this).attr('questionId');
	  api.onDeleteQuestion(id).then(ui.deleteQuestionSuccess).catch(ui.deleteQuestionFailure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var indexOfSurveys = function indexOfSurveys(data) {
	...
	var showAuthUserSurveys = function showAuthUserSurveys(data) {
	...
	var surveyQuestions = function surveyQuestions(id) {
	  // console.log(id)
	  return $.ajax({
	    url: config.apiOrigin + '/questions',
	    method: 'GET',
	...
	var onDeleteQuestion = function onDeleteQuestion(id) {
	  return $.ajax({
	...
	var update = function update(surveyId, data) {
	...
	var answerQuestion = function answerQuestion(data, id) {
	  return $.ajax({
	    url: config.apiOrigin + '/questions/' + id,
	    method: 'PATCH',
	    data: {
	...
	var getQuestionData = function getQuestionData(id) {
	  return $.ajax({
	    url: config.apiOrigin + '/questions/' + id,
	    method: 'GET'
	  });
	...
	module.exports = {
	  ...
	  indexOfSurveys: indexOfSurveys,
	  showAuthUserSurveys: showAuthUserSurveys,
	  // userIndex,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var showQuestionHB = __webpack_require__(16);
	var unauthUserSurveyHB = __webpack_require__(36);
	var authUserSurveyHB = __webpack_require__(37);
	var answerableSurveyHB = __webpack_require__(38);
	var editableSurveyHB = __webpack_require__(39);
	var showQuestionHeaderHB = __webpack_require__(40);

	var createSurveySuccess = function createSurveySuccess(response) {
	  store.surveyID = response.survey.id;
	  var showQuestionHtml = showQuestionHeaderHB({ surveys: response });
	...
	var createQuestionSuccess = function createQuestionSuccess(response) {
	  $('.alert').text('You Have Added The Question "' + response.question.prompt + '" to This Survey');
	  var showQuestionHtml = showQuestionHB({ questions: response });
	  $('#content').append(showQuestionHtml);
	  document.getElementById('create-question').reset();
	};

	var indexOfSurveysSuccess = function indexOfSurveysSuccess(data) {
	  $('#handlebar-target').text('');
	  $('form').hide();
	  if (data.surveys.length === 0) {
	    $('.alert').text('There are no Surveys to Take.');
	  }
	  var unauthUserSurveyHtml = unauthUserSurveyHB({ surveys: data.surveys });
	  $('#handlebar-target').html(unauthUserSurveyHtml);
	};

	var indexOfSurveysFailure = function indexOfSurveysFailure(surveyId) {
	  $('.alert').text('Unable to Retrieve Data.');
	};

	var showAuthUserSurveysSuccess = function showAuthUserSurveysSuccess(data) {
	  $('form').hide();
	  $('.alert').text('');
	  var answerableSurveyHtml = authUserSurveyHB({ surveys: data.survey });
	  $('#handlebar-target').html(answerableSurveyHtml);
	};

	var showAuthUserSurveysFailure = function showAuthUserSurveysFailure(data) {
	  $('.alert').text('Unable to Retrieve Data.');
	};

	var destroySuccess = function destroySuccess() {
	  api.showAuthUserSurveys().then(showAuthUserSurveysSuccess).catch(showAuthUserSurveysFailure);
	  $('.alert').text('Successful Survey Deletion');
	};

	var destroyFailure = function destroyFailure(data) {
	  $('.alert').text('Survey Deletion Failed');
	};

	var updateSuccess = function updateSuccess(surveyId) {
	  api.showAuthUserSurveys().then(showAuthUserSurveysSuccess).catch(showAuthUserSurveysFailure);
	};
	...
	var takeSurveySuccess = function takeSurveySuccess(data) {
	  ...
	  var answerableSurvey = answerableSurveyHB({ questions: data.question });
	  $('#handlebar-target').html(answerableSurvey);
	};

	var takeSurveyFailure = function takeSurveyFailure(data) {
	  $('.alert').text('Unable to Return Questions From Server');
	};

	var surveyQuestionsSuccess = function surveyQuestionsSuccess(data) {
	  var editableSurvey = editableSurveyHB({ questions: data.question });
	  $('#handlebar-target').html(editableSurvey);
	};

	var surveyQuestionsFailure = function surveyQuestionsFailure(data) {
	  $('.alert').text('Unable to Return Questions From Server');
	};

	var answerSuccess = function answerSuccess(data) {
	  var targ = document.getElementById(data._id);
	  $(targ).hide();
	  $('.alert').text('Your Answer has Been Logged!');
	};

	var answerFailure = function answerFailure(data) {
	  $('.alert').text('Failure to Log Answers');
	};

	var extractVal = function extractVal(data) {
	  var layer = Object.values(data);
	  layer = Object.values(layer[0]);
	  return layer[0];
	};

	var getQuestionDataSuccess = function getQuestionDataSuccess(data) {
	  var yes = 0;
	  var no = 0;
	  for (var i = 0; i < data.question.results.length; i++) {
	...
	var getQuestionDataFailure = function getQuestionDataFailure(data) {
	  $('.alert').text('Failure to Return Question');
	};

	var deleteQuestionSuccess = function deleteQuestionSuccess(data) {
	  var targ = document.getElementById(data._id);
	  $(targ).hide();
	  $('.alert').text('Question Deleted');
	};

	var deleteQuestionFailure = function deleteQuestionFailure(data) {
	...
	module.exports = {
	  ...
	  indexOfSurveysSuccess: indexOfSurveysSuccess,
	  indexOfSurveysFailure: indexOfSurveysFailure,
	  destroySuccess: destroySuccess,
	  destroyFailure: destroyFailure,
	  updateSuccess: updateSuccess,
	  updateFailure: updateFailure,
	  showAuthUserSurveysSuccess: showAuthUserSurveysSuccess,
	  showAuthUserSurveysFailure: showAuthUserSurveysFailure,
	  takeSurveySuccess: takeSurveySuccess,
	  takeSurveyFailure: takeSurveyFailure,
	  answerSuccess: answerSuccess,
	  answerFailure: answerFailure,
	  surveyQuestionsSuccess: surveyQuestionsSuccess,
	  surveyQuestionsFailure: surveyQuestionsFailure,
	  getQuestionDataSuccess: getQuestionDataSuccess,
	  getQuestionDataFailure: getQuestionDataFailure,
	  deleteQuestionSuccess: deleteQuestionSuccess,
	  deleteQuestionFailure: deleteQuestionFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr>\n        <td class=\"prompt-title\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.prompt : stack1), depth0))
	    + "</td>\n        <!-- <td><button surveyId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger delete-survey-button\">Delete</button></td> -->\n        <!-- <td><form class=\"update-survey-by-id-form\" surveyId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">\n            <input type=\"text\" id=\"update-survey-by-id-button\" name=\"survey[description]\">\n            <input type=\"hidden\" name=\"id\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">\n        <button surveyId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"submit\" task=\"text\" class=\"btn btn-info update-survey-button\">Edit</button> -->\n      </form>\n      </tr>\n\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return "<table id=\"survey-question-table\" class=\"table table-striped signed-in-view\">\n\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.questions : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</table>\n";
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr>\n        <td class=\"prompt-title\">"
	    ...
	    + "\" type=\"click\" class=\"btn btn-primary take-survey\">Take Survey</button></td>\n        <!-- <td><button surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger delete-survey-button\">Delete</button></td> -->\n        <!-- <td><form class=\"update-survey-by-id-form\" surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n            <input type=\"text\" id=\"update-survey-by-id-button\" name=\"survey[title]\">\n            <input type=\"hidden\" name=\"id\" value=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\"> -->\n        <!-- <button surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"submit\" task=\"text\" class=\"btn btn-info update-survey-button\">Edit</button> -->\n      </form>\n      </tr>\n";
	  ...
	  return "<table id=\"unauth-user-survey-table\" class=\"table table-striped signed-in-view\">\n\n <thead>\n    <tr>\n      <th>Survey Title</th>\n    </tr>\n  </thead>\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.surveys : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</table>\n";
	},"useData":true,"useBlockParams":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr>\n        <td class=\"prompt-title\">"
	    ...
	    + "</td>\n        <td><button surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-info view-questions-button\">View Survey</button></td>\n        <td><button surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger delete-auth-survey-button\">Delete</button></td>\n      <td>\n        <form class=\"update-survey\" surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      <fieldset>\n        <input type=\"text\" name=\"survey[title]\" placeholder=\"Change Title\">\n        <input type=\"submit\" class=\"btn btn-info\" name=\"submit\" value=\"Update Survey\">\n      </fieldset>\n    </form>\n      </td>\n        <!-- <td><form class=\"update-survey-button\" surveyId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n            <input type=\"text\" id=\"rename-auth-survey-by-id-button\" name=\"survey[title]\">\n            <input type=\"hidden\" name=\"id\" value=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n        <button class=\"btn btn-info view-questions-button\" type=\"submit\">Rename</button>\n      </form>\n    </td> -->\n      </tr>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return "<!--for editable survey, can (rename - change edit button) survey, view survey rename to view questions, delete // mostly done, survey title\nauth-user-content\n-->\n<html>\n<table id=\"auth-user-survey-table\" class=\"table table-striped signed-in-view\">\n\n <!-- <thead>\n    <tr>\n      <th>"
	    + container.escapeExpression(container.lambda(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.title : stack1), depth0))
	    + "</th>\n    </tr>\n  </thead> -->\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.surveys : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</table>\n</html>\n";
	},"useData":true,"useBlockParams":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr id=\""
	    ...
	    + "\">\n        <td class=\"prompt-title\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.prompt : stack1), depth0))
	    + "</td>\n      <td>\n        <fieldset>\n        <form class=\"answer-question\" value=1 questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n          <button questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"submit\"  task=\"text\" class=\"btn btn-info answer-question-button\">yes</button>\n        </form>\n      </fieldset>\n      <fieldset>\n      <form class=\"answer-question\" value=0 questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n        <button questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"submit\" task=\"text\" class=\"btn btn-info answer-question-button\">no</button>\n      </form>\n    </fieldset>\n    </td>\n      </tr>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return "<!-- unauthorized, view survey for rando: loads question in survey, load in a way to answer not edit - -->\n\n<!-- unauthorized, view question for rando: loads question in survey, load in a way to answer not edit -->\n<table id=\"auth-user-survey-table\" class=\"table table-striped signed-in-view\">\n <thead>\n    <tr>\n      <th>Question Prompt</th>\n    </tr>\n  </thead>\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.questions : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</table>\n";
	},"useData":true,"useBlockParams":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr>\n        <td class=\"prompt-title\">"
	    ...
	    + "</td>\n        <!-- <td>"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "</td> -->\n        <!-- <td><button questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger delete-question-button\">Delete</button></td> -->\n        <td><button questionId=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger get-data\">Get answer data from this question</button></td>\n        <!-- <td><button surveyId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.survey : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger view-questions-button\">View Questions</button></td>\n        <td><button userId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.user : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"click\" class=\"btn btn-danger delete-auth-survey-button\">Delete Question</button></td>\n        <td><form class=\"update-survey-by-id-form\" userId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.user : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">\n            <input type=\"text\" id=\"update-auth-survey-by-id-button\" name=\"survey[title]\">\n            <input type=\"hidden\" name=\"id\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.user : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">\n        <button userId=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.user : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"submit\" task=\"text\" class=\"btn btn-info update-survey-button\">Edit Question</button>\n      </form> -->\n      </tr>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return "<!--show all user surveys, show all my surveys button, view this survey by owner, can (rename - change edit button) survey, view survey rename to view questions, delete, results (t/f) column // mostly done, survey title-->\n\n<table id=\"auth-user-survey-table\" class=\"table table-striped signed-in-view\">\n\n <thead>\n    <tr>\n    <th>"
	    + container.escapeExpression(container.lambda(((stack1 = (depth0 != null ? depth0.question : depth0)) != null ? stack1.prompt : stack1), depth0))
	    + "</th>\n    </tr>\n  </thead>\n  <h5 class=\"data-output\"></h5>\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.questions : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</table>\n";
	},"useData":true,"useBlockParams":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "<table id=\"survey-question-header-table\" class=\"table table-striped signed-in-view\">\n\n  <thead>\n    <tr>\n      <th>Survey Title: "
	    + container.escapeExpression(container.lambda(((stack1 = ((stack1 = (depth0 != null ? depth0.surveys : depth0)) != null ? stack1.survey : stack1)) != null ? stack1.title : stack1), depth0))
	    + "</th>\n    </tr>\n  </thead>\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.questions : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</table>\n";
	},"useData":true,"useBlockParams":true});
